...
async function передачаДанихДоВебСокету(віддаленийСокет, вебСокет, заголовокVless, даніДляПовтору = null, лічильникСпроб = 0) {
    ...
    try {
        ...
        if (!отриманіДані && даніДляПовтору && лічильникСпроб < МАКС_СПРОБ) {
            ...
            try {
                let новийСокет;
                if (даніДляПовтору.увімкнутиМаршрутизатор === 'socks5') {
                    новийСокет = await маршрутизаціяПідключення(даніДляПовтору.типАдреси, даніДляПовтору.адреса, даніДляПовтору.порт, даніДляПовтору.розібранаАдресаМаршрутизатора);
                } else if (даніДляПовтору.увімкнутиМаршрутизатор === 'http') {
                    новийСокет = await httpПідключення(даніДляПовтору.типАдреси, даніДляПовтору.адреса, даніДляПовтору.порт, даніДляПовтору.розібранаАдресаМаршрутизатора);
                } else {
                    новийСокет = await connect({ hostname: даніДляПовтору.серверПеренаправлення, port: даніДляПовтору.портПеренаправлення }, { allowHalfOpen: true });
                }
...
async function httpПідключення(типАдреси, віддаленаАдреса, віддаленийПорт, розібранаАдреса) {
    ...
    try {
        while (true) {
            ...
            const текстВідповіді = new TextDecoder().decode(буферВідповіді);
            ...
            if (текстВідповіді.includes('\r\n\r\n')) {
                ...
                if (заголовки.startsWith('HTTP/1.1 200') || заголовки.startsWith('HTTP/1.0 200')) {
                    ...
                    if (позиціяЗаголовка < буферВідповіді.length) {
                        ...
                        потікДаних.pipeTo(writable).catch(() => {});
                        
                        // @ts-ignore
                        сокет.readable = readable;
                    }
                } else {
                    throw new Error(`Помилка HTTP підключення: ${заголовки.split('\r\n')[0]}`);
                }
                break;
            }
        }
...
async function отриманняДанихМаршрутизатора(адреса) {
    ...
    if (частинаДо) {
        ...
        if (частини.length !== 2) {
            throw new Error('Недійсний формат адреси маршрутизатора');
        }
